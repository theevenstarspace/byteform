...
Byteform provides variable-length integer types that use a base-128 encoding scheme. These types are useful for efficiently encoding integers that vary widely in size, as smaller values use fewer bytes.

### uVarInt

[`uVarInt`](/api/variables/uVarInt.md) represents an unsigned variable-length integer. It automatically determines the number of bytes needed based on the value's magnitude.

```typescript
import { uVarInt } from '@evenstar/byteform';

const schema = uVarInt;
```

**Characteristics:**
- **Encoding**: Base-128 with continuation bits
- **Range**: 0 to 2^53 - 1 (JavaScript Number.MAX_SAFE_INTEGER)
- **Size**: Variable, typically 1-9 bytes depending on value

**Example values and their byte representation:**
- `0` → 1 byte: `[0x00]`
- `127` → 1 byte: `[0x7f]`
...
[`iVarInt`](/api/variables/iVarInt.md) represents a signed variable-length integer. It uses a similar encoding scheme but reserves one bit for the sign.

```typescript
import { iVarInt } from '@evenstar/byteform';

const schema = iVarInt;
```

**Characteristics:**
- **Encoding**: Base-128 with continuation bits and sign bit
- **Range**: −(2^53 − 1) (JavaScript Number.MIN_SAFE_INTEGER) to 2^53 - 1 (JavaScript Number.MAX_SAFE_INTEGER)
- **Size**: Variable, typically 1-9 bytes depending on value
